...
getAllServicesNamesToRoom(roomName:any):Observable<any>{
  return this.http.get<any>(`${this.baseUrl}api/v1/receptionist/services-room?roomName=${roomName}`);
}
  addReservation(data: any, roomName: any, confirm: boolean = false): Observable<any> {
    return this.http.post(
      `${this.baseUrl}api/v1/receptionist/roomreservation?roomName=${roomName}&confirm=${confirm}`,
      data
    );
  }
  ...
  updateReservation(resId: number, data: any, confirm: boolean = false): Observable<any> {
    return this.http.put(
      `${this.baseUrl}api/v1/receptionist/update-reservation-by-id?reservationId=${resId}&confirm=${confirm}`,
      data
    );
  }
}
